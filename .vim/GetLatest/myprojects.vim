...
" Last Change: Fri Feb 27 15:23:35 CET 2009
" Version:     0.0.5
...
if !exists('myprojects_enable')
	...
	function s:extractFunctionsFromLine(line)
		let functions = {}
		...
		if line != ''
			...
			while index <= 12
				if s:hasFunction('F' . index, a:line)
					let function = substitute(line, '.*\s\+F' . index . '="\([^"]\+\)".*', '\1', '')
	...
	function s:extractFunctions(line)
		...
		while line > 0 && indent(line) > 0 && len(functions) < 12
			let line -= 1
		...
		if name == ''
			...
			if getftype(path) != 'dir'
				...
				if cd != '.' && getftype(cd) != 'dir'
					...
					call s:appendFolder(s:buildFolder(name, 0, path, 1, cd, cd != '', filter, filter != '', s:defineFunctions(), {}), line)
	...
	function s:refresh(line)
		...
		let functions = s:extractFunctionsFromLine(line)
	...
	function s:getFolderAttributes(line)
		...
		if line > 0
			...
			if folderIndent != -1
				let paths[s:getPath(line)] = {'path': s:extractPathFromLine(line), 'cd': s:extractCdFromLine(line), 'filter': s:extractFilterFromLine(line), 'functions': s:extractFunctionsFromLine(line)}
	...
	function s:defineFunctions()
		...
		while index >= 1 && index <= 13
			...
			while index <= 12
				let list = add(list, index . '. F' . index . (!has_key(functions, index) ? '' : ' ' . functions[index]))
				let index += 1
			endwhile
			...
			if index >= 1 && index <= 13
				let mapping = input('Mapping for F' . index . ': ')

				if mapping != ''
					let functions[index] = mapping
				endif
